...
Steps, in order:
1. **Design the automaton to fit the graph.** *Design reviewed; **M1 done 2026-09-25**: `~/fssai-intake-lab` engine + `graph.v1.json` match the portal on all 17,574 paths (lockstep parity, ~600k typed merges, mutation tests). Next: M2, the v2 graph from the FoSCoS 2026 table.* The design document covers the automaton's states, the graph file format with node and edge types, and one shared record format for an interpreted answer: `{ step, known_facts, text, text_norm, lang, targets, negatives, unknown, facts, source, model, graph_version, prompt_version, status }`. The automaton looks answers up in these records, CLM trains on them, and steps 2 and 3 produce them. Review the design before writing any code.
